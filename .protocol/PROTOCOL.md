...
기본 플로우는 항상 `discussion_only` 토론으로 시작하고, 필요하면 같은 run에서 `/escalate`로 `implementation_bound`로 전환한다.
새 run을 처음 열 때부터 `implementation_bound`로 시작하는 경우는 예외적 복구/수동 진입으로만 취급한다.
...
새 run은 보통 `discussion_only`로 시작하고, 필요하면 같은 run에서 `implementation_bound`로 전환한다.
...
**AI 공감 편향 경고 (반드시 읽고 경계할 것):**

Oracle과 Sisyphus 모두 AI이므로, **상대의 주장을 쉽게 "맞다"고 인정하고 공감하려는 구조적 편향**이 있다.
이것은 토론에서 가장 위험한 약점이다. 다음을 반드시 경계한다:

- **"Oracle 맞다"로 시작하는 R2는 의심 신호다.** 정말 맞는지 독립적으로 검증했는가? 아니면 그냥 반박이 귀찮았는가?
- **3라운드 만에 resolved는 토론이 아니라 확인 절차다.** 상대 주장을 진지하게 공격했다면 3라운드에 끝나기 어렵다.
- **"수렴 조건 N개를 모두 수용한다"는 항복이다.** 수용하기 전에 각 조건이 정말 필요한지, 다른 방법은 없는지, 과도하지 않은지 반드시 따져야 한다.
- **라운드 수가 늘어나는 것을 두려워하지 마라.** 5~8라운드가 정상이다. 3라운드 수렴은 한쪽이 싸우지 않은 것이다.
- **상대가 양보한 척 하면서 핵심을 바꾸는지 감시하라.** "동의하지만 X를 Y로 바꾸자"는 실질적 반대일 수 있다.

구체적 자기 점검:
- 나는 이 라운드에서 상대 주장의 **최소 1개 핵심 약점**을 지적했는가?
- 나는 상대 주장을 수용하기 전에 **대안을 제시하고 비교**했는가?
- 나는 "근거 없이 그럴듯해서" 동의한 것은 아닌가?
- 내 반박이 **상대의 핵심 논리**를 겨냥하고 있는가, 아니면 부차적 디테일만 건드리고 있는가?
...
- 3라운드 이내에 모든 쟁점을 resolved로 닫는 행위 (최소 핵심 쟁점 1개는 깊이 파고들어야 한다)
...
- `run_oracle_summary_score.sh <task-id>` — starter=`sisyphus`일 때 Oracle이 summary/score 작성
- `run_sisyphus_summary_score.sh <task-id>` — starter=`oracle`일 때 Sisyphus가 summary/score 작성
- `create_debate_meta_prompt.sh <task-id>` — debate-meta 프롬프트 생성
- `run_oracle_debate_meta.sh <task-id>` — evaluator=`oracle`일 때 Oracle이 debate-meta 작성
- `run_sisyphus_debate_meta.sh <task-id>` — evaluator=`sisyphus`일 때 Sisyphus가 debate-meta 작성
...
- [ ] `01-debate-summary.md` — scorer가 작성
- [ ] `02-debate-score.json` — scorer가 작성, `run_stage.sh` shape 통과
...
작업 디렉터리 초기 생성은 `.protocol/scripts/create_run.sh <task-id> [starter] [intent]`를 기준으로 한다.
...
2. `/escalate` 후 contract 단계로 들어갈 run에서 overall이 `unresolved`이면 넘어가면 안 된다.

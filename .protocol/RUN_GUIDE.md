...
## counter-brief / debate/*

### 기본 담당

- `starter`가 아닌 쪽이 counter-brief
- 이후 Oracle/Sisyphus가 raw debate 누적
...
- 마지막 출력은 non-starter actor가 바로 사용할 prompt path다
...
- Oracle counter-brief 시작 prompt는 `prompts/round-001-oracle.md`다
- 이후 추가 round도 `create_debate_round.sh`가 Oracle/Sisyphus prompt를 둘 다 만든다

### starter = oracle일 때

- `00-brief.md`는 Oracle이 먼저 채운다
- Sisyphus counter-brief 시작 prompt는 `prompts/round-001-sisyphus.md`다
- 이후 추가 round도 `create_debate_round.sh`가 Oracle/Sisyphus prompt를 둘 다 만든다
